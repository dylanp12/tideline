...
/// Build an in-memory router with an explicit record store (record tests).
fn router_full(backend: Arc<dyn Backend>, auth: AuthConfig, events: Arc<Events>) -> Router {
    let auth = Arc::new(auth);
    let verifier = Arc::new(KeyVerifier::new());
    let tlr = tlr_state(backend.clone(), auth.clone(), verifier.clone());
    let state = AppState {
        backend,
        auth,
        verifier,
    ...
    Router::new()
        ...
        .with_state(state)
        .merge(crate::tlr::routes::router(tlr))
        .layer(DefaultBodyLimit::max(MAX_BODY))
        // A panic should fail one request, not drop the connection silently.
...
async fn metrics(State(st): State<AppState>) -> String {
    ...
    streams + &crate::tlr::metrics::render()
}

// --- TLR/1 wiring ---------------------------------------------------------

/// Adapts the Cloud key verifier to the record layer's tenant resolver, so the
/// `/v1` routes never reach into the control-plane client directly.
struct CloudTenants {
    verifier: Arc<KeyVerifier>,
    auth: Arc<AuthConfig>,
}
...
impl crate::tlr::routes::TenantResolver for CloudTenants {
    async fn resolve(&self, token: &str) -> Option<String> {
        let url = self.auth.verify_url.as_ref()?;
        self.verifier
            .verify(url, &self.auth.verify_secret, token)
            .await
            .map(|t| t.namespace())
    }
}

/// The record store for `/v1`: a SQLite file at `TIDELINE_RECORD_DB`, or an
/// ephemeral in-memory store when unset (so tests write no files).
fn default_tlr_store() -> Arc<dyn crate::tlr::store::TlrStore> {
    use crate::tlr::sqlite::SqliteTlrStore;
    match std::env::var("TIDELINE_RECORD_DB")
        .ok()
        .filter(|s| !s.is_empty())
    {
        Some(path) => Arc::new(SqliteTlrStore::open(&path).expect("open TIDELINE_RECORD_DB")),
        None => Arc::new(SqliteTlrStore::in_memory().expect("open in-memory record store")),
    }
}
...
fn tlr_state(
    ...
    auth: Arc<AuthConfig>,
    verifier: Arc<KeyVerifier>,
) -> crate::tlr::routes::TlrState {
    let mode = crate::tlr::auth::AuthMode {
        publish_token: auth.publish_token.clone(),
        subscribe_token: auth.subscribe_token.clone(),
        cloud_verify: auth.verify_url.is_some(),
    };
    let tenants = Arc::new(CloudTenants {
        verifier,
        auth: auth.clone(),
    });
    crate::tlr::routes::TlrState::new(
        default_tlr_store(),
        Arc::new(crate::tlr::keys::SigningKeyring::from_env()),
        mode,
        backend,
    )
